...
Nitro 构建显式使用 `node-server` preset，数据库层按运行时选择驱动：Docker 的 Bun runner 使用 `bun:sqlite`，外部 Node 环境使用 `better-sqlite3`。产物没有静态 `bun:` import，因此 Node ESM loader 可以正常加载。
